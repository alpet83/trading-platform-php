...
const instanceBackLink = computed(() => {
  ...
  return {
    path: '/instance',
    query: {
      hostId: String(route.query.hostId),
    },
  }
})
onMounted(async () => {
...
async function fetchData() {
  const res = await useApiRequest(`/api/instance/error/${route.query.bot}`, {
    method: "GET",
    params: route.query.hostId ? { hostId: String(route.query.hostId) } : undefined,
  });
