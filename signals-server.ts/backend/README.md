...
TRADING_DB_AUTH_URL=mysql://user:password@localhost:3306/trading
```
...
Дополнительно для доступа к торговой базе (MariaDB/MySQL) используется переменная `TRADING_DB_AUTH_URL`.
Она нужна для users-only рефакторинга и прямой работы backend с таблицей `chat_users` без PHP-прослойки.
