...
=======
    ...
    // Discard any warnings/notices emitted by includes so SVG stays valid.
    ob_clean();
    ...
    function fetch_recent_subscribers(mysqli_ex $db): array {
    $sql = "SELECT `remote_ip`, `remote_host`, DATE_FORMAT(`last_seen`, '%Y-%m-%d %H:%i:%s') AS `last_seen`, `hits`
>>>>>>> ce583c8 (hotfix: stabilize lib include/require paths in signals scripts)
    FROM `signals_stats`
    WHERE `endpoint` = ?
...
=======
    ...
    $stmt->bind_param('s', $endpoint);
    if (!$stmt->execute()) {
        // Missing stats table or query error: return empty list and keep SVG healthy.
        $stmt->close();
        return [];
    }
