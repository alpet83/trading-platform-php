...
function on_shutdown() {
    ...
    if (is_object($g_bot) && $g_bot instanceof TradingCore && !$g_bot->aborted) {
        $stage = $g_bot->runtime_stage;
        try {
            $g_bot->LogError("~C91#BREAK:~C00 script execution interrupted at stage '$stage', on_shutdown...\n");
        } catch (\Throwable $e) {
            // DB may already be closed during teardown — suppress to avoid noisy PHP errors
        }
...
function sig_term_safe($signo, $siginfo) {
    ...
    if ($bot) {
        ...
        if (15 == $signo) {
            throw new Exception("SIGTERM received");
        }
    } else {
...
class TradingCore {
    ...
    public string $runtime_stage = 'init'; // updated at each init phase; visible in on_shutdown log
    ...
    public function __construct() {
        ...
        if (isset($env['impl_name'])) {
            $this->impl_name = $env['impl_name'];
        } // may be overrided after/before this constructor
    ...
    public function Initialize($mysqli) {
        ...
        $this->runtime_stage = 'load-config';
        ...
        // Record startup immediately so web-ui shows the bot even before the first Update() cycle.
        // ON DUPLICATE KEY resets ts_start to the new session time; restarts are counted via {exchange}__events.
        $ts_now = date(SQL_TIMESTAMP);
        $mysqli->try_query("INSERT INTO `bot__activity`(ts, ts_start, applicant, account_id)
            VALUES('$ts_now', '$ts_now', '{$this->impl_name}', {$engine->account_id})
            ON DUPLICATE KEY UPDATE ts_start = '$ts_now'");
        ...
        $this->runtime_stage = 'init-engine';
        $t_start = time();
        ...
        $this->runtime_stage = 'load-signals';
        $t_start = time();
    ...
    protected function CheckSaveFunds() {
        if ($this->total_funds < 0 || $this->total_btc < 0) {
            return;
        }
        $engine = $this->Engine();
        ...
        // POSITION_COEF_SCALE: reference deposit size used to compute the max allowed
        // position_coef. Default 10000 suits $100K+ accounts. For small deposits
        // (e.g. $100) set POSITION_COEF_SCALE=100 to avoid coef being zeroed out.
        $pc_scale = max(1.0, (float)(getenv('POSITION_COEF_SCALE') ?: 10000));
        $quants = $this->total_funds / $pc_scale;
        if ($this->total_funds > $pc_scale) {
        ...
        if ($coef > $limit && $this->total_funds > 0 && 0 == date('i') % 10) {
            $this->send_event('ALERT', sprintf(
                "For account %s, position coef %.3f is too high due funds level %.2f, set to %.3f",
                $acc_id,
                $this->configuration->position_coef,
                $this->total_funds,
                $limit
            ));
    ...
    public function Run() {
        ...
        while ($this->conn_attempts < 10) {
            ...
            if ($this->CheckDBConnection()) {
                ...
                $ws_engine = $this->Engine();
                if ($ws_engine instanceof WebsockAPIEngine) {
                    $ws_engine->drainWsBuffer();
                }
            }
            sleep(1);
            $prev_h = $hour;
    ...
    public function ProcessTasks() {
        ...
        if (!$mysqli->table_exists($tasks_table)) {
            return;
        }
        $acc_id = $engine->account_id;
        ...
        foreach ($tasks as $task) {
            ...
            if ('CANCEL_ORDER' == $task->action) {
                ...
                if ($status <= 0 || $status > 100) {
                    $this->LogMsg("~C93#TASK_DEFER:~C00 CANCEL_ORDER #%s deferred, exchange unavailable (status %d)", $task->param, $status);
                    continue; // keep task in DB, retry next cycle
                }
            }
            ...
            } elseif ('RESTART' == $task->action) {
                $this->Shutdown('restart-task');
            }
        }
    }
    ...
    public function EnsureBotTables(): void {
        ...
        // Keep existing bot tables in sync with new position anchor schema.
        $positions_table = $prefix . '__positions';
        $pos_history_table = $prefix . '__position_history';
        $mysqli->add_column($positions_table, 'avg_pos_price', 'DOUBLE NOT NULL DEFAULT 0', 'offset');
        $mysqli->add_column($pos_history_table, 'avg_pos_price', 'DOUBLE NOT NULL DEFAULT 0', 'value_qty');
    }

    /**
     * Сохраняет снимок контекста принятия торгового решения в таблицу {exchange}__exec_context.
     * Вызывается из Trade() после формирования решения по каждой паре.
     * Запись UPSERT'ится по (account_id, pair_id) — всегда одна строка на пару.
     */
    public function PersistExecContext(TradingContext $ctx, string $action, string $detail = ''): void {
        $mysqli = $this->mysqli;
        if (!$mysqli) {
            return;
        }
        ...
        $mysqli->try_query(
            "INSERT INTO `$table` (account_id, pair_id, ts, context_json)
             VALUES ($acc, $pid, NOW(3), '$json')
             ON DUPLICATE KEY UPDATE ts = NOW(3), context_json = '$json'"
        );
    }
    ...
    protected function CheckRedudancy(): bool {
        ...
        if (!$row) {
            return $this->SetActiveRole(true, 'registered');
        }  // first bot in array
        ...
        if ($mhost != $hostname || $mpid != $pid) {  // master is another instance, lets check them
            ...
            if ($hostname != $mhost) {
                ...
                $this->LogMsg(
                    "~C91 #WARN:~C00 due master instance 0%x from %s, errors = $errors, last active time '%s' (+$elps) status = '%s', switching self $pid to master role. Trade skip %d",
                    $mpid,
                    $mhost,
                    $mts,
                    $last_st,
    ...
    public function CheckReplication(): bool {
        ...
        if (is_array($hosts)) {
            foreach ($hosts as $row) {
                $id = $row['id'];
                if ($id == $engine->host_id) {
                    continue;
                }
                $elps = time() - strtotime($row['last_alive']);
                if ($elps < 120) {
                    return true;
                } // есть другие активные хосты
            }
        }
        return false;
    }
        ...
        if ($elps > $this->update_period || $startup) {
            ...
            while ($do_update) {// once cycle
                try {
                    ...
                    $this->ProcessTasks(); // process before availability check: RESTART must work even if exchange down
                    ...
                    if ($engine instanceof WebsockAPIEngine && $engine->wsTickersFresh()) {
                        $load_t = true; // WS data current — SaveTickersToDB uses in-memory pairs_info
                    } else {
                    ...
                    $ws_ready = ($engine instanceof WebsockAPIEngine) && !$startup && $engine->isWsReady();
                    // ws_cache_valid: broader — covers short reconnect cooldowns when in-memory
                    // positions/orders from WS are still current (up to 5-min grace period)
                    $ws_cache_valid = ($engine instanceof WebsockAPIEngine) && !$startup
                        && ($engine->isWsReady() || $engine->wsTickersFresh(300));
                    $load_p = $ws_cache_valid ? 0 : $engine->LoadPositions();   // depended from ticker info

                    $load_o = $ws_cache_valid ? 0 : $engine->LoadOrders($startup);
                    ...
                    if (false !== $load_p && intval($load_o) >= 0 && $load_t > 0 && $this->active) {
                        ...
                        if (!$startup && $api_key_trade_allowed && $this->TradingAllowed() && ($hour < 23 || $minute <= 58) && $load_p >= 0) { // no trades before script reload
                            ...
                            if (0 == $this->trade_skip) {
                                $engine->ProcessMM();
                            }   // обработать заявки маркет-мейкера
                            $this->Trade(); // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                        }
                        ...
                        $ws_state = ($engine instanceof WebsockAPIEngine)
                            ? sprintf(' ws_ready=%d cache=%d', (int)$ws_ready, (int)$ws_cache_valid)
                            : '';
                        $this->LogError("~C91#FAIL:~C00 trading disabled, due load status = %d, %d, %d, active = %d%s",
                            $load_p === false ? -1 : $load_p,
                            $load_o === false ? -1 : $load_o,
                            $load_t, $this->active, $ws_state);
                    }
            ...
            // Use explicit runtime dir (BOT_RUNTIME_DIR env) rather than relying on CWD glob
            $bot_runtime_dir = getenv('BOT_RUNTIME_DIR') ?: getcwd();
            $debug_files = glob($bot_runtime_dir . '/*debug.inc.php') ?: [];
            $fname = count($debug_files) > 0 ? $debug_files[0] : '';
            ...
            if ($master) {
                $this->ProcessTasks(); // always process tasks (e.g. RESTART must work with trade_enabled=false)
            }
    ...
    protected function SavePID(bool $force = false) {
        ...
        if (file_exists($fpid_name) &&
            is_resource($this->pid_file_handle) && !$force) {
            return;
        }
    ...
    public function Shutdown(string $reason) {
        $this->LogError("~C31#WARN:~C00 shutdown signal received, reason: $reason"); // must be logged in bot last_errors table 
        $this->active = false;
        $this->aborted = true;
    }
