...
class OrderInfo {
    ...
    public function __construct() {
        $this->raw_set = array('id' => -1, 'host_id' => 0, 'predecessor' => 0, 'ts' => 0, 'ts_fix' => 0, 'account_id' => 0, 'pair_id' => 0, 'batch_id' => 0, 'signal_id' => 0,
                                'avg_price' => 0, 'avg_pos_price' => 0, 'init_price' => 0, 'price' => 0, 'amount' => 0, 'buy' => true, 'matched' => 0,
    ...
    public function __set($key, $value) {
        ...
        if ('in_position' == $key && 0 == $this->matched) {
            $this->out_position = $value;
        } // синхронизация, пока нет исполнения

        if ('matched' == $key) {
            $matched_prev = floatval($this->matched);
            $matched_next = floatval($value);
            $delta_matched = $matched_next - $matched_prev;

            $pos_before = floatval($this->out_position);
            $delta_pos = $delta_matched * $this->TradeSign();
            $pos_after = $pos_before + $delta_pos;
            $this->out_position = $pos_after; // delta-change

            // Recalculate only on actual matched change that increases absolute position.
            if ($this->registered && $delta_matched != 0.0 && abs($pos_after) > abs($pos_before)) {
                $fill_price = max(floatval($this->avg_price), floatval($this->price));
                $avg_before = floatval($this->avg_pos_price);
                [$avg_after] = PositionMath::advance_anchor($pos_before, $avg_before, $delta_pos, $fill_price);
                if (abs($avg_after - $avg_before) > 1e-12) {
                    $this->avg_pos_price = $avg_after;
                }
            }
        }
        ...
        }  // в этот набор сохранять только legacy параметры, они сохранятся в БД
        else {
    ...
    public function __toString() {
        ...
        $res = sprintf(
            'id#%d %s %s %s @ %5G st=%s, batch=%d, m=%s, r=%s, f:0x%03x, №%s',
            $this->id,
            $this->Side(),
            $this->pair,
            $amount,
    ...
    public function IsFixed() {
        ...
        if ($this->status == OST_LOST) {
            return false;
        }
        if (false !== array_search($this->status, ARCHIVED_STATUSES)) {
            return true;
        }
        ...
        if (false !== array_search($this->status, PENDING_STATUSES)) {
            return false;
        }
        if ($this->IsFilled()) {
            return true;
        ...
        if (OrderStatusCode::FILLED == $code && $pending <= 0) {
            return true;
        } // TODO: epsilon from config

        if (OrderStatusCode::TOUCHED == $code && $pending > 0) {
            return $this->onwer ? $this->owner->is_fixed : false;
        }  // недобитки попадают в matched, из-за отмены например или смещения

        if ($this->owner) {
            $this->owner->TradeCore()->LogError(
                "~C91#WARN:~C00 unknown status for order %s %s, epsilon = %f, pending = %f, ff = 0x%x ",
                strval($this),
                $this->comment,
                $epsilon,
                $pending,
    ...
    public function Import(array $record): int {
        ...
        if (!isset($this->matched_prev)) {
            $this->matched_prev = $this->matched;
        } // for tracking changes start
    ...
    public function Register(OrderList $order_list): bool {
        ...
        if (!$mysqli->try_query($query)) {
            ...
            if ('none' != $prev_owner) {
                $line = sprintf(
                    tss()." #REGISTERED: in %-15s from %-15s #%d %s %s \n",
                    $new_owner,
                    $prev_owner,
                    $this->id,
                    $this->order_no,
    ...
    protected function set_flags(int $ff) {
        ...
        if ($ff & OFLAG_FIXED && false !== array_search($this->status, $pend)) {
            active_bot()->LogError(
                "~C91#ERROR:~C00 try set fixed flag for pending order: %s, source %s, from %s",
                strval($this),
                json_encode($this->source_raw),
                format_backtrace()
            );
            return;
        }
    ...
    public function set_status(string $value, bool $force = false) {
        ...
        try {
            ...
            if (0 == $this->matched && (OrderStatusCode::FILLED == $code || OrderStatusCode::TOUCHED == $code)) {
                $core->LogError(
                    "~C91#WARN:~C00 try set ~C97'$value'~C00 status for order %s with matched = 0, from:\n %s",
                    strval($this),
                    format_backtrace()
                );
                return false;
            ...
            if ($fixed && false === $can_mod &&
                ...
                if ($force) {
                    $core->LogError(
                        "~C91#WARN:~C00 forced change status to %s (%s) for fixed order %s, source %s, from:\n %s",
                        $value,
            ...
            $this->owner->TradeCore()->LogError(
                "~C91#EXCEPTION:~C00 set status '%s' for %s, error %s from\n~C97".$e->getTraceAsString(),
                $value,
                strval($this),
                $e->getMessage()
            );
    ...
    public function OnUpdate(string $op = 'load'): bool {
        ...
        if (false !== array_search($op, ['submit', 'post', 'move'])) {
            $this->flags |= OFLAG_ACTIVE;
        }  // успешно выполненные операции выставления или перемещения заявки, дают знать что она активна

        if (!$this->IsFixed()) {
            $this->checked = date_ms(SQL_TIMESTAMP3);
        }  // метка времени начнет устаревать, после отмены или исполнения заявки
...
class OrderList implements ArrayAccess, Countable, Iterator {
    ...
    protected $imp_fields    = 'id, predecessor, ts, account_id, pair_id, batch_id, avg_price, avg_pos_price, init_price, price, amount, buy, matched, order_no, flags, in_position, status, comment, updated';
    ...
    public function __construct(TradingEngine $engine, string $name, bool $fixed = false) {
        ...
        $this->AddField('avg_pos_price', 'DOUBLE NOT NULL DEFAULT 0', 'avg_price'); // avg entry price of active position segment
        ...
        if (strlen($code) > 10) {
            ...
            if (false == strpos($code, "PARTITION p$p_max") && $size > $limit) {
                try {
                    ...
                    $query .= implode(",\n  ", $parts).");\n";
                    $core->LogMsgсф("~C91#WARN:~C00 trying parititioning table with query:\n %s", $query);
    ...
    protected function AddField(string $field, string $params, string $after) {
        ...
        if ($mysqli->column_exists($table_name, $field)) {
            return;
        }

        $code = $mysqli->show_create_table($table_name);
        if (false === $code) {
            $core->LogError("~C91#WARN:~C00 can't retrieve CREATE TABLE for %s: %s", $table_name, $mysqli->error);
            return;
        }

        if ($mysqli->add_column($table_name, $field, $params, $after, true)) {
            return;
        }

        $core->LogMsg("~C97 #SHOW_CREATE_TABLE:~C00 %s, attempt to upgrade failed: %s", $code, $mysqli->error);
    }
        ...
        foreach ($this->orders as $info) {
            if ($info->pair_id == $pair_id && $info->Elapsed('created') < $period) {
                ...
                if (isset($info->$k)) {
                    ...
                    $val = $info->matched * $info->TradeSign();
                } elseif ($k == 's_amount') {
                    $val = $info->amount * $info->TradeSign();
                }
                $map[$info->id] = $val;
            }
        }
                ...
                foreach ($rows as $row) {
                    if (isset($row['id'])) {
                        ...
                        if (!in_array($load_st, [OST_ACTIVE, OST_PROTO, OST_NEW]) && $this->is_fixed) {
                            $row['flags'] |= OFLAG_FIXED;
                        } // force fixed flag for non-pending orders
    ...
    public function SaveToDB(string $sql_params = '') {
        ...
        try {
            ...
            if (!$this->is_fixed) {
                $mysqli->try_query("DELETE FROM `$table` $sql_params; -- {$this->list_name}->SaveToDB");
            } // TODO: this cleanup can destroy old orders
            ...
            $volatile = 'status,batch_id,signal_id,matched,avg_price,avg_pos_price,ts_fix,updated,flags,in_position,out_position';
            ...
            $core->LogOrder(
                "~C96#PERF({$this->list_name}.SaveToDB):~C00 in list %d orders, total count changed from %d to %d ",
                $this->count(),
                $before,
                $after
            );
            ...
            if ($this->is_fixed && $before > $after) {
                ...
                if ($inserted < count($rows)) {
                    ...
                    $core->LogError(
                        "~C91#WARN:~C00 after save fixed list %s lost %d orders [%d => %d], inserted %d / %d",
                        $this->list_name,
                        $lost,
                        $before,
                        $after,
