...
function scan_zero_saldo(array &$orders): ?array {
    ...
    $core->LogMsg(
        "~C95#CLEANUP:~C00 for %s vector has saldo %.3f, matched values %s, rest %d orders",
        strval($oinfo),
        $saldo,
        implode(',', $vml),
        count($orders)
...
function dbg_array_push(&$dest, $value) {
    ...
    if ($upd < $count + 1) {
        active_bot()->LogError(
            "~C91 #ERROR:~C00 dbg_array_push %s failed increase result count $count => %d",
...
class ExternalSignal {
    ...
    // Set when user reduces amount vs previous value; cleared once Bias() reaches ~0.
    protected $amount_reduction = false;
    ...
    public function __get($key) {
        ...
        if (!property_exists($this, $key)) {
            $this->core->LogError(
                "~C91#ERROR:~C00 for signal %d trying get unknown property %s, but available %s",
                $this->id,
                $key,
                json_encode(array_keys($this->raw))
            );
        }
        return null;
    }
    ...
    public function __set($key, $value) {
        ...
        if ($key === 'amount' && isset($this->raw['amount']) && $value < $this->raw['amount'] && $value > 0) {
            $this->amount_reduction = true;
        }
        $this->raw[$key] = $value;
    }
    ...
    public function __toString() {
        ...
        if (!$this->engine) {
            return sprintf("ID:%d, pair_id=%d, $side uninitialized now", $this->id, $this->pair_id);
        }
        $this->ticker_info = $ti = $this->engine->TickerInfo($this->pair_id);
        $res = '';
        if ($ti) {
            $res = sprintf(
                "ID:%d @%s, active:%d, %s %s Δ amount:%s, coef:%.5f",
                $this->id,
                $this->account_id,
                intval($this->active),
                $side,
                $ti->pair,
                $ti->FormatAmount($this->amount, Y, Y),
                $this->open_coef
            );
        } else {
            $res = sprintf("ID:%d @%s, %s %s Δ amount = %10G", $this->id, $side, $this->account_id, $this->pair_id, $this->amount);
        }
        if ($this->flags & SIG_FLAG_SL && $this->stop_loss > 0) {
            $res .= ", stop_loss:{$this->stop_loss}";
        }
        if ($this->flags & SIG_FLAG_LP && $this->limit_order > 0) {
            $res .= ", limit_order: {$this->limit_order}";
        }
        if ($this->flags & SIG_FLAG_TP && $this->take_order > 0) {
            $res .= ", take_order: {$this->take_order}";
        }
        if ($this->closed) {
            $res .= ',~C31 CLOSED!~C00';
        }
    ...
    public function AddBatch(int $id, string $ctx = '') { // добавление пачки заявок
        if ('construct' == $ctx) {
            return;
        } // преодоление рекурсии, при отработки конструктора пачки
    ...
    public function AdjustCloseAmount(bool $buy, float $amount): float {
        ...
        foreach ($orders as $oinfo) {
            if ($oinfo->buy == $buy) {
                continue;
            } // нужны только встречные заявки, которые открывали смещение
            $m = $oinfo->matched;
            if ($prev + $m > $amount) {
                continue;
            } // перебор
    ...
    public function AddOrder(mixed $order, string $ctx = ''): bool {
        if (is_object($order)) {
            if ($order->IsFixed() && 0 == $order->matched) {
                return false;
            } // не добавлять отмененные
            if ($order->flags & OFLAG_RESTORED) {
                return false;
            }
            ...
            if ($order->IsOutbound() || $elps > $very_aged) {
                return false;
            }

            if ($order->signal_id > 0 && $order->signal_id != $this->id) { // разрешать только адаптацию, чужие сигналы не вписывать
                $this->core->LogError(
                    "~C91#ERROR(AddOrder):~C00 order %s signal_id %d mismatch for signal %s, stack:\n %s",
                    strval($order),
                    $order->signal_id,
                    strval($this),
                    format_backtrace()
            ...
            if (strpos($ctx, 'Trade') !== false) {
                ...
                if ($side_need != $order->Side()) {
                    $sig_side  = $this->buy ? 'buy' : 'sell';
                    // Counter-direction order is expected during user-initiated amount reduction.
                    $is_reduce = $this->amount_reduction && ($order->Side() !== $sig_side);
                    $wmsg = "~C91#WARN:~C00 wrong side order %s for %s (bias %+.4f, sig=%s), ctx: %s";
                    if ($is_reduce) {
                        $this->core->LogMsg($wmsg, $order->Side(), strval($this), $bias, $sig_side, $ctx);
                    } else {
                        $this->core->LogError($wmsg, $order->Side(), strval($this), $bias, $sig_side, $ctx);
                    }
                }
            }
    ...
    public function AverageEntryPrice() {
        ...
        foreach ($orders as $oinfo) {
            if (is_object($oinfo)) {
                ...
                if (0 == $oinfo->matched || $this->buy != $oinfo->buy) {
                    continue;
                }
    ...
    public function CalcRelative(string $key): float {
        if (!isset($this->raw[$key]) || !is_object($this->ticker_info)) {
            return 0;
        }
        $ti = $this->ticker_info;
        $base = $this->raw[$key];
        if ($base > $ti->last_price * 0.2) {
            return $base;
        } // non-relative
        $entry = $this->AverageEntryPrice();
        if (0 == $entry) {
            $entry = $ti->last_price;
        } // пока не было открывающих сделок, разрешить вход указав стоп ниже текущей (для лонга).
    ...
    public function Cleanup(bool $force = false) {
        ...
        if ($this->PendingAmount() > 0 && !$force) {
            return 'have pending';
        }
        if ($total < $this->min_clean) {
        ...
        if ($bias_cost > $core->ConfigValue('min_order_cost', 100) && !$force) {
            return 'have delta';
        }  // при активном расхождении, не трогать

        $cdp_list = $this->CurrentDeltaPos(Y, N);  // для сохранения истории
        if (count($cdp_list) < $this->min_clean) {
            return 'not enough orders 2';
        }
        ...
        if (is_object($this->last_matched)) {
            unset($cdp_list[$this->last_matched->id]);
        } // последняя исполненная заявка не участвует в очистке, она важна для грид-ботов

        foreach ($cdp_list as $oid => $pos) {
            $n = count($check_list);
            $ap = abs($pos);
            if (0 != $cdp_start) {
                if ($oid == $this->limit_order) {
                    continue;
                } // не трогать заявку открытия, для очистки
            }
            ...
            if ($ap <= $min_amount) { // абсолютного нуля на практике можно не дождаться, издержки округлений и комиссионных съемов
                $force_before = $n;
            }  // до этого индекса - все под нож
            $check_list [] = $oid;
        }

        if (0 == count($check_list)) {
            return 'no enough matched';
        } // все заявки нужные
        sort($check_list);
        $removed = [];
        ...
        if (abs($all_saldo) < $min_amount) {
        ...
        } else { // дальше можно сканировать список исполненных заявок до получения нулевого сальдо, и помечать анигилировавшие. Слишком долгая жизнь сигнала приведет к сотням заявок в списке
            foreach ($match_map as $list) {
                ...
                if (0 == count($buys) || 0 == count($sells)) {
                    continue;
                } // анигилировать нечему
                $core->LogMsg(
                    "~C95#CLEANUP:~C00 for %s found orders with same matched amount %.6f, %d buys, %d sells",
                    strval($this),
                    $list[0]->matched,
                    count($buys),
                    count($sells)
        ...
        if (0 == count($forget)) {
            ...
            while ($list = scan_zero_saldo($all_orders)) { // попытка итеративной сборки группы с общим нулевым сальдо
                ...
                $core->LogMsg(
                    "~C95#CLEANUP:~C00 for %s found group of %d orders with small saldo %f, matched values %s",
                    strval($this),
                    count($list),
                    array_sum($vl),
                    implode(',', $vl)
        ...
        if ($diff > max(abs($cdp_start), abs($cdp_end)) * 0.01) {
            $core->LogMsg(
                "~C91#CLEANUP_WARN:~C00 signal %s total forgot %d / %d orders (until %d),  currrent delat pos changed %.6f => %.6f",
                strval($this),
                count($removed),
                $total,
                $force_before,
    ...
    public function CurrentDeltaPos(bool $native = true, bool $ret_saldo = true) {
        ...
        foreach ($ids as $n => $id) {
            ...
            if (0 == $oinfo->signal_id) {
                $oinfo->signal_id = $this->id;  // Fix: was $id (order ID), must be signal ID
                $oinfo->OnUpdate('signal-link');
            }

            if ($oinfo->signal_id != $this->id) {
                $core->LogError(
                    "~C91#ERROR(CurrentDeltaPos):~C00 order %s signal_id %d mismatch for signal %s - removing as lost",
                    strval($oinfo),
                    $oinfo->signal_id,
                    strval($this)
                );
            ...
            $saldo += $sm;
            $tag = ' ';
            if ($this->limit_order == $id) {
                $tag = '~C07L';
            }
            if ($this->take_order == $id) {
                $tag = '~C07T';
            }
            $story [] = format_color(
                "$tag$bs%d@%s:%8s=%-6s;$sfx",
                $id,
                $fprice,
                $ti->FormatAmount($sm, Y, Y),
    ...
    public function TargetDeltaPos(bool $native = true, bool $raw = false): float { // возвращает дельту позиции в нативном значении, при полной или частичной активации
        ...
        if ($this->limit_order > 0) {
            if (!$this->limit_info) {
                return 0;
            } // не инициализирована заявка открытия
            $info = $this->limit_info;
            ...
            if (!$this->active) {
                return $matched;
            }  // если заявка не исполнена в достаточной степени, то это и есть целевая позиция
        }
    ...
    public function TryAdoption() {
        ...
        foreach ($ml as $oinfo) {
            ...
            if (!$oinfo->IsFixed() || $flags != 0 || $oinfo->matched == 0) {
                continue;
            }
            ...
            if ($pdiff > $oinfo->price * 0.02) {
                continue;
            } // экстремально отличается
            $batch_id = $oinfo->batch_id;
            $batch = null;
            if ($batch_id > 0) {
                $batch = $engine->GetOrdersBatch($batch_id, 3);
                if (is_object($batch) && $batch->parent > 0) {
                    continue;
                } // такие сигналы нельзя трогать
            }
    ...
    public function LastBatch() {
        ...
        if ($last >= 0) {
            return $this->gen_batches[$last];
        } // предполагается, что ранние сигналы просто протухли
        return false;
    }
    public function LocalAmount(bool $native = true, bool $raw = false, bool $apply_coef = false): float {
        ...
        if ($apply_coef) {
            $base *= $this->open_coef;
        } // лучше его применить до округления в нативные единицы
    ...
    public function PendingOrders(bool $skip_lp = true, bool $skip_tp = true): array {
        ...
        foreach ($this->gen_orders as $oid) {
            $oinfo = $owner->FindOrder($oid, $this->pair_id, true);
            if (!$oinfo || $oinfo->IsFixed()) {
                continue;
            }
            if ($oinfo->IsLimit() && $oinfo->id == $this->limit_order && $skip_lp) {
                continue;
            }
            if ($oinfo->IsLimit() && $oinfo->id == $this->take_order && $skip_tp) {
                continue;
            }
            $res [$oid] = $oinfo;
        }
    ...
    public function Import(mixed $src): bool {
        ...
        if (is_array($src)) {
            ...
            if (!isset($src['account_id'])) {
                $src['account_id'] = $acc_id;
            }  // remote loaded signals have no account binding
    ...
    public function LoadFromDB(int $id): bool {
        ...
        if (is_object($info) && $info->count > 0) { // поиск реального изменения, которое дало исполнение заявки
            $this->last_changed = strtotime($info->last_order_ts);
        } //  $mysqli->select_value('ts', $m_table, "WHERE (signal_id = $id) AND (account_id = {$this->account_id}) AND (ts < '{$this->ts}') ORDER BY ts DESC");  // latest matched order
    ...
    public function UpdateBTCPrice() {
        ...
        if ($this->last_changed > 0 && $changed_day < $core->session_start) {
            $bp = $core->CalcVWAP(BITCOIN_PAIR_ID, $changed_day, 24 * 60);
        } // using 24H avg from day before
        ...
        if ($bp != $this->btc_price) {
            ...
            $core->LogMsg(
                "~C94#DBG:~C00 for $class %s btc_price updated to $%.2f, change day %s vs session start %s ",
                strval($this),
                $bp,
                $changed_day,
                $core->session_start
    ...
    public function Update() {
        ...
        if ($this->flags & SIG_FLAG_LP) {
            ...
            if ($this->limit_order <= 0 || !$info) {
                ...
                $this->active = ($info->matched > $info->amount * 0.5);
            } // хоть немножко, половиночку
        }
        ...
        if ($this->active && $this->take_profit > 0 && $this->tp_order_info) {
            ...
            if ($oinfo->matched > 0) {
                $this->stop_loss = 0;
            } // stop price is not actual anymore, fixing open_coef
            if (0 == $this->limit_price) {
                $this->closed |= $oinfo->IsFixed() && ($oinfo->matched > 0);
            } // сигнал можно только один раз закрыть, если он без перезагрузки
        }
        ...
        if ($this->amount_reduction && abs($this->Bias()) < 1e-8) {
            $this->amount_reduction = false;
        }
    ...
    public function SaveToDB() {
        ...
        // Trim comment to DB column limit before any INSERT/UPDATE attempt
        $this->raw['comment'] = trim_comment($this->raw['comment'] ?? '');
...
class GridSignal extends ExternalSignal {
    ...
    public function __toString() {
        if (!$this->engine) {
            return sprintf("ID:%d, pair_id=%d, uninitialized now", $this->id, $this->pair_id);
        }
        $this->ticker_info = $ti = $this->engine->TickerInfo($this->pair_id);
        $res = '';
        if ($ti) {
            $res = sprintf(
                "ID:%d @%s, active:%d, %s Δ mult:%f, amount:%s, levels: %d, L:%s, H:%s, cp: %s",
                $this->id,
                $this->account_id,
                intval($this->active),
                $ti->pair,
    ...
    public function LoadFromDB(int $id): bool {
        ...
        if ($res && is_null($this->last_matched)) {
            ...
            if (!is_null($id)) {
                ...
                $this->core->LogMM(
                    "~C31#GRID_WARN:~C00 last matched order for %s not loaded with query %s, result is NULL. May no matched orders yet. Grid lived %.3f hours.",
                    strval($this),
                    $mysqli->last_query,
                    $live_t / 3600.0
                );
        ...
        if ($res) {
            ...
            foreach ($this->gen_orders as $oid) {
                ...
                if ($oinfo->IsFixed()) {
                    continue;
                } // нельзя в список активных пихать исполненные
                $m = [];
            ...
            $this->core->LogMM("~C91#WARN_GRID_LOAD:~C00 failed for #$id %s: %s", strval($this), $this->last_error);
        } // may is new
    ...
    public function Report(string $prefix) {
        ...
        $core->LogMM(
            "$prefix~C00 for %s CDP = %s, last matched [%s], actual orders %s",
            strval($this),
            $cdp,
            strval($this->last_matched),
            json_encode($map)
        );
    }
...
class OffsetSignal extends ExternalSignal {
    ...
    public function LocalAmount(bool $native = true, bool $raw = false, bool $apply_coef = false): float {
        ...
        $core->LogError(
            "~C91#WARN:~C00 from offset %.5f for %s scale factor = %.7f, can't calc result pos",
            $res,
            strval($this),
            $coef
        );
...
class SignalFeed implements ArrayAccess, Countable {
    ...
    public function CalcPosition(int $pair_id, bool $target = true, bool $native = true, bool $raw = false) {
        ...
        foreach ($this->signals as $sig) {
            ...
            if (!$sig->active) {
                continue;
            } // только активные сигналы сальдировать для текущей позиции
            ...
            if ($pair_id == $dbg_pair) {
                $core->LogMsg(
                    " ~C94#CALC_POS_ADD($tt):~C00 $type %s, + %s = %s",
                    strval($sig),
                    $dtxt,
                    $stxt
                );
        ...
        if ($ti && $native && $cnt && $saldo != $prev) {
            $this->core->LogMsg(
                "~C96#CALC_POS_CHG:~C00 processed %2d / %3d active signals for pair %10s $tt position result %s => %s",
                $cnt,
                count($this->signals),
                $ti->pair,
    ...
    public function CalcPrices() { // рассчет цен для триггеров
        ...
        foreach ($used_pairs as $pair_id => $tinfo) {
            ...
            $rows = $mysqli->select_rows(
                'ask,bid,last,ROUND(UNIX_TIMESTAMP(ts)/60) as m',
                $table,
            ...
            if ($vwap_good) {
                ...
                $core->LogMsg(
                    "~C94#AVG_LAST:~C00 pair %9s, ll = %10s, mix %10s, hh = %10s, vwap = %10s, periods = %d / %d, used prices %s",
                    $tinfo->pair,
                    $tinfo->FormatPrice($lowest),
                    $mix,
                    $tinfo->FormatPrice($highest),
                    $tinfo->FormatPrice($vwap),
                    $period_slow,
                    $period_fast,
                    json_encode($prices)
                );
    ...
    public function CollectSignals(string $select, int $pair_id, bool $only_active = true): array {
        ...
        foreach ($this->signals as $sig) {
            if ($only_active && !$sig->active) {
                continue;
            }
            if ($sig->closed || $sig->pair_id != $pair_id && $pair_id > 0) {
                continue;
            }
    ...
    public function DispatchOrder(OrderInfo $oinfo, string $ctx = 'DispatchOrder') {
        ...
        if ($oinfo->batch_id <= 0 || $oinfo->IsOutbound()) {
            return false;
        }
    ...
    public function GetUnfilled(int $pair_id, bool $prefered_buy, float $min_cost = 100): ?ExternalSignal { // получение сигнала для пары, который не полностью исполнен
        ...
        ksort($this->signals);
        foreach ($this->signals as $sig) {
            if ($sig->pair_id != $pair_id || $sig->closed || $sig->flags & SIG_FLAG_GRID) {
                continue;
            }
            ...
            if ($dbg) {
                $core->LogMsg(
                    "$tag#DBG(GetUnfilled):~C00 signal %s, delta cost %9.2f, orders %2d, cdp = %10f, tdp = %10f, last scale = %s",
                    strval($sig),
                    $cost,
                    $cnt,
                    $cp,
                    $tp,
                    $this->engine->last_scale
                );
            } //*/
            if ($ready) {
    ...
    public function LoadFromDB() {
        ...
        $this->signal_info = $mysqli->select_map(
            'signal_id, COUNT(ts) as count, MIN(ts) as first_order_ts, MAX(ts) as last_order_ts, MIN(id) as first_order, MAX(id) as last_order',
            $ord_table,
            "WHERE (account_id = $acc_id) AND (signal_id > 0) GROUP BY signal_id",
            MYSQLI_OBJECT
        );
        if (tp_debug_mode_enabled()) {
        ...
        if ($start > 0) {
            ...
            foreach ($this->orders_cache as $id => $sig_id) {
                $this->orders_map[$id]  = $engine->FindOrder($id, 0, true);
            } // for fast access
        }
    ...
    public function FindByOrder(OrderInfo $oinfo): ?ExternalSignal {
        ...
        if (is_object($oinfo)) {
            foreach ($this->signals as $sig) {
                $ti = $sig->ticker_info;
                if ($sig->closed || !is_object($ti)) {
                    continue;
                }
                if ($ti->pair_id != $oinfo->pair_id) {
                    continue;
                } // optimization
                if (false !== $sig->FindOrder($oinfo->id)) {
                    return $sig;
                }
            }
        }
    ...
    public function LoadSignals(): int {
        ...
        foreach ($this->signals as $sig_id => $sig) {
            if ($sig_id >= MAX_OFFSET_SIG) {
                $sig->closed = true;
            } // mark all as closed, before overwrite
            else {
                $loaded[$sig->id] = true;
        ...
        foreach ($this->signals as $sig_id => $sig) {
            ...
            if (!isset($loaded[$sig_id]) && !$sig->closed) {
                ...
                $sig->closed = true;
                $sig->SaveToDB();  // persist closed=true so LoadFromDB won't reload this signal
                foreach (array_unique($sig->gen_orders) as $oid) {
                    $oinfo = $engine->FindOrder($oid, $sig->pair_id, false);
                    if ($oinfo && $oinfo->signal_id == $sig->id) {
                        $oinfo->signal_id = 0;  // detach: clear signal link so TryAdoption can pick up the order
                        $oinfo->OnUpdate('signal-closed');
                    }
                }
                $sig->gen_orders = [];
            }
        ...
        $this->core->LogMsg(
            "~C93#PERF(LoadSignals):~C00 imported %d / %d acceptable signals from %s, total = %d, closed = %d, used_pairs = %d, skipped = %s, failed = %d",
            $imported,
            $acceptable,
            $url,
            count($this->signals),
            $ccnt,
            count($used_pairs),
            json_encode(array_keys($skipped)),
            $failed
        );
        return $imported;
    }
