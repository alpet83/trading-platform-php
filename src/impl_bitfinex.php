...
final class BitfinexEngine extends WebsockAPIEngine {
    public const API_BASE_PUBLIC  = 'https://api-pub.bitfinex.com/';
    public const API_BASE_PRIVATE = 'https://api.bitfinex.com/';
    ...
    // chanId => pair_id (populated when subscription is confirmed)
    private array                   $ws_chan_map      = [];

    public $api_key_fails = 0;

    // Paper trading mode: symbols remapped to tTEST variants in Bitfinex paper trading.
    // Controlled by 'exchange_profile' param in the bot config table (any non-main profile = paper mode).
    private bool $bfx_paper = false;

    public function __construct(object $core) {
        parent::__construct($core);
        $this->exchange = 'Bitfinex';

        // bot_manager passes credentials as BITFINEX_API_KEY / BITFINEX_API_SECRET
        // (strtoupper(exchange) pattern). BFX_* prefix is used for other settings only.
    ...
    // -------------------------------------------------------------------------
    // Paper-trading symbol helpers
    // -------------------------------------------------------------------------

    /**
     * Transform a native Bitfinex symbol to its paper-trading equivalent.
     * tBASEQUOTE -> tTESTBASETESTQUOTE
     */
    private function ToApiSym(string $sym): string {
        if (!$this->bfx_paper || $sym === '') return $sym;
        if (str_starts_with($sym, 'tTEST')) return $sym; // already paper
        static $quotes = ['USDT', 'USDC', 'USD', 'BTC', 'ETH', 'GBP', 'JPY', 'DAI', 'EOS', 'XMR', 'LTC', 'ETC', 'ETP'];
        if ($sym[0] === 't') {
            $body = substr($sym, 1);
            foreach ($quotes as $q) {
                if (str_ends_with($body, $q)) {
                    $base = substr($body, 0, strlen($body) - strlen($q));
                    if ($base !== '') return 'tTEST' . $base . ':TEST' . $q;
                    break;
                }
            }
        }
        $this->LogMsg('~C91#BFX_PAPER_WARN:~C00 cannot map to paper symbol: %s', $sym);
        return $sym;
    }

    /**
     * Reverse-transform a paper symbol from private API response to native.
     * tTESTBASETESTQUOTE -> tBASEQUOTE
     */
    private function FromApiSym(string $sym): string {
        if (!$this->bfx_paper || !str_starts_with($sym, 'tTEST')) return $sym;
        $body = substr($sym, 5); // strip 'tTEST'
        $pos  = strpos($body, ':TEST');
        if ($pos === false) return $sym;
        return 't' . substr($body, 0, $pos) . substr($body, $pos + 5); // ':TEST' is 5 chars
    }
    ...
    protected function wsSend(string $data): bool {
        ...
        try {
            // @ suppresses vendor-level fwrite warnings on broken-pipe; exception still propagates
            @$this->ws_client->send($data);
            return true;
        } catch (\Throwable $E) {
            $this->LogMsg("~C91#WS_SEND_FAIL:~C00 %s", $E->getMessage());
            $this->ws_client = null; // force wsIsConnected()=false → immediate reconnect
            return false;
        }
    }
    ...
    protected function wsPing(): void {
        // Bitfinex WS v2 ping-pong via event
        $this->wsSend(json_encode(['event' => 'ping']));
    }

    protected function wsPong(string $payload = ''): void {
        // Bitfinex does not require explicit pong from client
    }

    // =========================================================================
    // WebSocket protocol — Bitfinex WS v2 (array-based)
    // =========================================================================
    ...
    protected function wsBuildAuthMessage(): string {
        $nonce  = strval(time_ms());
        $payload = 'AUTH' . $nonce;
        $secret = base64_decode(implode('', $this->secretKey) . '==');
        $sig    = hash_hmac('sha384', $payload, $secret);
        return json_encode([
            'event'       => 'auth',
            'apiKey'      => $this->apiKey,
            'authSig'     => $sig,
    ...
    protected function wsSubscribeAll(): void {
        if (!$this->ws_authenticated) {
            ...
            if ($this->wsSend($auth_msg)) {
                $this->LogMsg("~C93#WS_AUTH:~C00 Bitfinex WS auth message sent");
            }
            return;
        }
        ...
        $this->LogMsg("~C93#WS_SUBSCRIBE:~C00 Bitfinex WS subscribed to %d ticker channels", $count);
        $this->ws_subscribe_after = 0;
    }

    /**
     * Bitfinex WS v2 dispatcher.
     * Event frames are stdClass objects; data frames are PHP arrays.
     */
    protected function wsDispatch(mixed $data): void {
        // --- Event frame (object, e.g. {"event":"subscribed",...}) ---
        if (is_object($data)) {
            $event = strval($data->event ?? '');

            if ($event === 'pong') {
                $this->ws_last_ping = time();
                $this->ws_heartbeat_frame = true; // suppress packet count in wsReadFrame
                return;
            }
            ...
            if ($event === 'auth') {
                ...
                if ($status === 'OK') {
                    ...
                    $this->LogMsg("~C92#WS_AUTH_OK:~C00 Bitfinex WS authenticated, uid=%s", $data->userId ?? '?');
                    ...
                    $msg = $data->msg ?? json_encode($data);
                    $this->LogMsg("~C91#WS_AUTH_FAIL:~C00 %s", $msg);
            ...
            if ($event === 'subscribed') {
                $chan_id  = intval($data->chanId ?? 0);
                $channel = strval($data->channel ?? '');
                $symbol  = strval($data->symbol ?? '');
                if ($channel === 'ticker' && $chan_id > 0) {
                    $pmap = $this->pairs_map_rev ?? [];
                    if (isset($pmap[$symbol])) {
                        $this->ws_chan_map[$chan_id] = $pmap[$symbol];
                        $this->LogMsg("~C93#WS_SUB_CONFIRM:~C00 ticker %s → chanId %d → pair_id %d", $symbol, $chan_id, $pmap[$symbol]);
                    }
                }
                return;
            }
            ...
            if ($event === 'error') {
                $code = $data->code ?? 0;
                $msg  = $data->msg ?? json_encode($data);
                $this->LogMsg("~C91#WS_ERROR:~C00 [%d] %s", $code, $msg);
                return;
            }

            if ($event === 'info') {
                // server version / maintenance info
                if (isset($data->code) && $data->code == 20060) {
                    // Bitfinex maintenance mode started
                    $this->LogMsg("~C91#WS_MAINTENANCE:~C00 Bitfinex entered maintenance mode");
                    $this->wsReconnect('maintenance mode');
                }
                return;
            }
        ...
        // Heartbeat on any channel
        if ($data[1] === 'hb') {
            $this->ws_last_ping = time();
            return;
        }

        // Auth channel (chanId = 0): order/trade events
        if (0 === $chan_id) {
            $type = strval($data[1] ?? '');
            $payload = $data[2] ?? null;
            $this->wsHandleAuthEvent($type, $payload);
            $this->ws_priv_packets++;
            return;
        }

        // Registered ticker channel
        if (isset($this->ws_chan_map[$chan_id])) {
            $pair_id = $this->ws_chan_map[$chan_id];
            // Ticker data is an array of 10 numbers (or array-of-arrays on first snapshot)
            if (is_array($data[1])) {
                $ticker = $data[1];
                // Snapshot: array of arrays
                if (is_array($ticker[0])) {
                    $ticker = $ticker[0];
                }
                $this->wsOnTicker($pair_id, $ticker);
            }
            $this->ws_pub_packets++;
            return;
        }
    ...
    /**
     * Handle messages on the auth channel (chanId = 0).
     * Types: os (order snapshot), on (new), ou (update), oc (close),
     *        te (trade exec), tu (trade update).
     */
    private function wsHandleAuthEvent(string $type, mixed $payload): void {
        if (null === $payload) {
            return;
        }

        switch ($type) {
            case 'os': // order snapshot — array of order arrays
                if (is_array($payload)) {
                    $this->ImportOrders($payload, 'ws-os!', true);
                }
                return;

            case 'on': // new order placed
                if (is_array($payload)) {
                    $this->ImportOrders([$payload], 'ws-on!', true);
                }
                return;

            case 'ou': // order update
                if (is_array($payload)) {
                    $status = strval($payload[13] ?? '');
                    $is_open = (stripos($status, 'ACTIVE') !== false ||
                                stripos($status, 'PARTIALLY FILLED') !== false);
                    $this->ImportOrders([$payload], 'ws-ou!', $is_open);
                    // Mark filled pair for fast MM reaction on partial fills
                    if (stripos($status, 'PARTIALLY FILLED') !== false) {
                        $this->wsMarkFilledByRec($payload);
                    }
                }
                return;

            case 'oc': // order closed (filled or cancelled)
                if (is_array($payload)) {
                    $status = strval($payload[13] ?? '');
                    $this->ImportOrders([$payload], 'ws-oc!', false);
                    if (stripos($status, 'EXECUTED') !== false) {
                        $this->wsMarkFilledByRec($payload);
                    }
                }
                return;

            case 'te': // trade execution notification (fires before 'ou')
                if (is_array($payload)) {
                    // payload: [ID, PAIR, MTS, ORDER_ID, EXEC_AMOUNT, EXEC_PRICE, ...]
                    $sym = $this->FromApiSym(strval($payload[1] ?? ''));
                    $pmap = $this->pairs_map_rev ?? [];
                    if ($sym !== '' && isset($pmap[$sym])) {
                        $this->wsMarkFilledPair($pmap[$sym]);
                    }
                }
                return;

                // tu (trade update) — duplicate of te with settled fees, skip
            default:
                return;
        }
    }
    ...
    /**
     * Resolve pair_id from an order record array and call wsMarkFilledPair.
     */
    private function wsMarkFilledByRec(array $rec): void {
        $sym = $this->FromApiSym(strval($rec[3] ?? ''));
        $pmap = $this->pairs_map_rev ?? [];
        if ($sym !== '' && isset($pmap[$sym])) {
            $this->wsMarkFilledPair($pmap[$sym]);
        }
    ...
    /**
     * Update TickerInfo from WS ticker payload.
     * Bitfinex v2 ticker array:
     * [BID, BID_SIZE, ASK, ASK_SIZE, DAILY_CHANGE, DCR, LAST_PRICE, VOLUME, HIGH, LOW]
     */
    private function wsOnTicker(int $pair_id, array $t): void {
        if (count($t) < 8) {
            return;
        }
        ...
        $tinfo->bid_price  = (float)$t[0];
        $tinfo->ask_price  = (float)$t[2];
        $tinfo->last_price = (float)$t[6];
        $tinfo->daily_vol  = round((float)$t[7] * $tinfo->last_price, 4);
        $tinfo->OnUpdate();
        $this->ws_ticker_t = time();

        // Update BTC price if this is a BTC/USD-like pair
        $sym = array_search($pair_id, $this->pairs_map_rev ?? []);
        if ($sym && false !== strpos($sym, 'BTC') && false !== strpos($sym, 'USD')) {
            $pb = strpos($sym, 'BTC');
            $pu = strpos($sym, 'USD');
            if ($pu > $pb && $tinfo->last_price > 0) {
    ...
    public function Initialize() {
        ...
        // --- exchange profile setup (paper trading uses non-main profile) ---
        $core   = $this->TradeCore();
        ...
        $this->bfx_paper = ($profile_name !== 'main');
        if ($this->bfx_paper) {
            $this->LogMsg('~C93#BFX_PAPER:~C00 paper trading mode ACTIVE (profile: %s, symbols: tTEST...)', $profile_name);
            // Extend pairs_map_rev with paper-symbol aliases (tTESTETH:TESTUSD => id)
            // so that LoadPositions / WS handlers can match them directly without FromApiSym.
            foreach (array_keys($this->pairs_map_rev) as $sym) {
                $test_sym = $this->ToApiSym($sym);
                if ($test_sym !== $sym)
                    $this->pairs_map_rev[$test_sym] = $this->pairs_map_rev[$sym];
            }
        }
        ...
        if (is_array($res) && count($res) > 50) {
            ...
            $this->TradeCore()->LogError("~C91#ERROR:~C00 PrivateAPI user/info returned %s [%s]", $json, $this->AuthContext());
        }
    }
    ...
    public function CheckAPIKeyRights(): bool {
        ...
        if (error_test($res, 'apikey: invalid')) {
            throw new Exception(sprintf('#FATAL: Bitfinex API key rejected [%s]: %s', $this->AuthContext(), strval($json)));
        }

        if (!is_array($res) || count($res) < 10) {
            throw new Exception(sprintf('#FATAL: Bitfinex API key rights check failed [%s]: %s', $this->AuthContext(), strval($json)));
        }

        $this->LogMsg('~C93#AUTH_CHECK:~C00 Bitfinex key validated [%s], exchange does not expose read-only scope in current flow', $this->AuthContext());
        return true;
    }
    ...
    private function DecodeOrder($info, $rec) {
        ...
        if ($tif > time() && !$info->IsFixed()) {
            ...
            if ($timeout < 120) {
                $core->LogOrder(
                    "~C91#WARN:~C00 order %s will expired by %d seconds, elapsed = %.3f hours, tif = %s UTC",
                    strval($info),
                    $timeout,
                    $elps,
                    $info->runtime['expiration_ts']
                );
            }
        }
    ...
    private function ProcessOrderResult(OrderInfo $info, string $json, string $op): ?OrderInfo {
        ...
        if (200 == $curl_rcode &&
            ...
            $core->LogError(
                "~C91#FAILED~C00: API %s returned %s, last_nonce = %15.3G, curl rcode = %d, error_map = %s",
                $op,
                $json,
                $nonce / 1000.0,
                $curl_rcode,
                json_encode($info->error_map)
            );
    ...
    private function DecodeStatus($info, $src) {
        ...
        if (array_search($info->status, $can_mod) !== false) {
            while (count($stack) > 0) {
                ...
                if (strlen($st) >= 5) {
                    if ($st == 'canceled') {
                        $st = $info->SetFixedAuto();
                    } // автодетект статуса, в зависимости от исполненной части
                    else {
                        $info->status = $st;
                    }
    ...
    public function MoveOrder(OrderInfo $info, float $price, float $amount = 0): ?OrderInfo {
        ...
        if ('lost' === $res->status) {
            ...
            $info->price = $price;
        } // price modified, since order changed
    ...
    public function LoadTickers() {
        ...
        // Drain WS buffer first — skip REST polling when WS tickers are fresh
        $this->drainWsBuffer();
        if ($this->wsTickersFresh(30)) {
            return count($this->pairs_map_rev ?? []);
        }
        ...
        foreach ($list as $rec) {
            if (is_array($rec) && count($rec) > 15) {
                ...
                if ($max_order > MAX_ORDER_OFFSET && $clid > $max_order && !isset($this->outer_orders[$cid])) {
                    ...
                    $core->LogOrder(
                        "~C91#WARN:~C00 outrange client order id %d (max expected %d) for %s, total %d, source [%s]: ~C97".json_encode($rec, true),
                        $clid,
                        $max_order,
                        $desc,
                        count($this->outer_orders),
                        $source
                    ); // placed from terminal or other bot, must be ignored
                    continue;
                }
                ...
                try {
                    if (!$opened && 'active' == $info->status) {
                        $core->LogError(
                            "~C91#WARN:~C00 ImportOrders found active order %s after %s processing statuses: %s",
                            strval($info),
                            $source,
                            json_encode($this->dbg_statuses)
                        );
                        if ($info->matched > 0) {
                    ...
                    $core->LogError(
                        "~C91#ERROR:~C00 ImportOrders failed to process order %s, %s, statuses: %s",
                        strval($info),
                        $e->getMessage(),
                        json_encode($this->dbg_statuses)
                    );
    ...
    public function LookupOrder(OrderInfo $info): ?OrderInfo {
        ...
        if (isset($info->pair)) {
            $api_sym = $this->ToApiSym($info->pair);
            $url = "v2/auth/r/orders/{$api_sym}/hist";
        }
        $core = $this->TradeCore();
    ...
    public function LoadOrders(bool $force_all) {
        ...
        if (1 == count($sym_list)) {
            $strict = '/' . $this->ToApiSym($sym_list[0]);
        }
        $url = "v2/auth/r/orders/hist";
        $params = [];
        $list = false;
        ...
        if (0 == count($lost_orders)) {
            return $loaded;
        }
        $core->LogOrder(
            "#DBG: checking %d lost orders (pairs: %s), updated from history %s request, imported $hst",
            count($lost_ids),
            implode(',', $sym_list),
            json_encode($params)
        );
    ...
    public function NewOrder(TickerInfo $tinfo, array $proto): ?OrderInfo {
        ...
        $symbol = $this->ToApiSym($core->pairs_map[$tinfo->pair_id]);
        $amount = $info->amount;
        ...
        $order_type = 'LIMIT'; // always margin; EXCHANGE LIMIT (spot) must never be used
        $params = array('cid' => $info->id + $id_origin, 'type' => $order_type, 'symbol' => $symbol, 'price' => strval($info->price),
    ...
    private function RequestPrivateAPI($path, $params, $method = 'POST', $sign = true) {
        ...
        while (date('s') % 2 != $this->host_id) { // wait for personal second slot (0=even,1=odd)
            usleep(10000);
        }
