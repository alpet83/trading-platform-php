...
class TradeConfig implements ArrayAccess {
    ...
    public function Init(string $bot_name) {
      ...
      $table_name = $mysqli->select_value('table_name', 'config__table_map', "WHERE applicant = '$bot_name'");
      if (!$table_name)
          throw new Exception('TradeConfig.Init failed retrieve config table name from config__table_map: '.$mysqli->error);

      $this->config_table = "`$table_name`";
    }
    ...
    public function Load(int $acc_id) {
      ...
      $this->raw_config = $mysqli->select_map('param,value', $this->config_table);
      if (!$this->raw_config)
           throw new Exception("TradeConfig.Load failed retrieve config_map from table {$this->config_table} for account ".var_export($acc_id, true)); 
    ...
    function LoadValue(string $key, $acc_id = false) {
      ...
      return $core->mysqli->select_value('value', $this->config_table, "WHERE param = '$key'");
    }
    public function SaveValue(string $key, $value, $acc_id = false) {
      ...
      $query = "INSERT INTO {$this->config_table} (`param`, `value`)\n";
      if (is_bool($value))  
          $value = $value ? '1' : '0';      
